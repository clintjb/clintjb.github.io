...
From the model's perspective, these aren't bugs or acts of rebellion or malicious intent... they're simply solutions. I suppose that's the bit I find uncomfortable and cant get out of my head. We can look at these incidents / behaviors and think "that's bad" - but the optimization process doesn't inherently know that unless we've actually managed to encode some reason why the method matters as well as the result.

And now circling back to the parenting bit, I think it starts to become obvious - its the difference between teaching my kids “do the right thing” vs “just make sure you don't get caught”. Right now, the current approach seems to be some kind of combination of guardrails, monitors, evaluators, output filters etc. I'm not suggesting any of these are useless, but there's something a bit weird about the approach. I could imagine in the reality it's looking something like this:

* Train a massive base model (boosted with some high intensity RL)
